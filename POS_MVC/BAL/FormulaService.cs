...
namespace Service.Service
{
    public class FormulaService : IFormulaService
    {
        ...
        public decimal CalcFormula(Formula formula, IDictionary<string, decimal> salaryItemsValue, IEnumerable<SalaryItem> salaryItems)
        {
            ...
            {
                decimal Op1 = 0;
                SalaryItem salaryItem = salaryItems.Where(x => x.Id == formula.FirstSalaryItemId).FirstOrDefault(); // _salaryItemService.GetObjectById(formula.FirstSalaryItemId.GetValueOrDefault());
                try
                {
