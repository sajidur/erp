...
namespace RexERP_MVC.Controllers
{
    public class CustomerController : Controller
    {
        ...
        public ActionResult YesrClosed(int Id)
        {
            var duesummary = year.CustomerWiseYearClosed(Id);
        ...
        public ActionResult Details(int id)
        {
            ...
            try
            {
                result.Limit = db.GetById(id).Limit;
            }
            catch (Exception ex)
            {
        ...
        public ActionResult GetDetailsByLedgerId(int ledgerId)
        {
            if (ledgerId == 0)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            Customer customer = db.GetByLedgerId(ledgerId);
            if (customer == null)
            {
                return HttpNotFound();
            }
